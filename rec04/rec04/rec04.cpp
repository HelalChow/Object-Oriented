...
//  rec04
//  Helal Chowdhury
//  Created by Helal Chowdhury on 2/22/19.
...
public:
    ...
    void movesInWith(Person& newRoomate) {
        if(roomie && newRoomate.roomie) {cerr << "You already have a roomate";}
        else if(&newRoomate == this) {cerr << "You cannot move in wiht urslef";}
        else{
...
//    p = 0x00007ffeefbff5ec;
    
    cout << "p points to where " << *p << " is stored "  << endl;
    
    *p = -2763;
    cout << "p points to where " << *p << " isstored\n";
...
//    p   = pD;
    ...
    *pDyn = 17;
    ...
    SpeakerSystem ss1, ss2;
    SpeakerSystem* s1p = nullptr;
    SpeakerSystem* s2p = nullptr;
    *s1p = ss1;
    *s2p = ss2;
    
    //s1p->attachSpeakers(ss2);
    //s2p->attachSpeakers(ss1);
